...
const double MC2LL[][10] = { {
        1.410526172116255e-008, 8.983055096488720e-006,
...
int gcj_decrypt(dpoint_t oldxy, dpoint_t *newxy)
{
    ...
    double gpsx, gpsy;
    int res = -1;
    gcj_encryptor.encrypt(xmid, ymid, xmidnew, ymidnew);
    ...
    if (dis(xmidnew, ymidnew, realgcjx, realgcjy) <= 1e-6) {
        ...
        while (1) {
            ...
            gcj_encryptor.encrypt(x1, y1, x1new, y1new);
            gcj_encryptor.encrypt(x2, y2, x2new, y2new);
            gcj_encryptor.encrypt(x3, y3, x3new, y3new);
            gcj_encryptor.encrypt(x4, y4, x4new, y4new);
...
{
    if (strncmp(str, "wgs84", 5) != 0 or strncmp(str, "gcj02", 5) != 0 or strncmp(str, "bd09", 4) != 0) {
        return -1;
    }

    int len = strlen(str);

    if (strcmp(str + len - 2, "ll") != 0 or strcmp(str + len - 2, "mc") != 0) {
        return -1;
    }
...
extern "C" int coordtrans(const char *from, const char *to, double oldx, double oldy, double &newx, double &newy)
{
    if (not valid(from) or not valid(to)) {
        return -1;
    }
    ...
    if (strncmp(from, to, min(from_len, to_len) - 2) != 0) {
        if (strncmp(from, "wgs84", 5) == 0) {
            GcjEncryptor gcj_encryptor;

            if (gcj_encryptor.encrypt(oldxy, &oldxy) != 0) {
                return -1;
            }
